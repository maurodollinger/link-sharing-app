...
const CustomDropdown = ({ field, form, onClick }: CustomDropdownProps) => {
  ...
  const label =
    IconOptions.find((option) => option.value === currentOption)?.label ||
    "Github";
  ...
  return (
    <div className="relative">
      <div className="flex w-full relative items-center">
        ...
        <label
          ref={labelRef}
          className="custom-input custom-select w-full"
...
const NewLinkInputs = ({ data }: NewLinkInputsProps) => {
  ...
  return (
    <div
      id={`link-class-${data.index}`}
      className="bg-light-grey rounded-xl p-5"
    >
      <div className="flex justify-between">
        ...
        <button
          ...
          onClick={() => removeUserLink(data.id)}
        >
          Remove
        </button>
      </div>
...
export default function FormCreator({ userlinks }: { userlinks: Link[] | [] }) {
  ...
  const initialValues = userlinks.map((link) => ({
    id: link.id,
  ...
  useEffect(() => {
    if (lastAddedIndex !== null) {
      const timeout = setTimeout(() => {
        const newLinkInput = document.getElementById(
          `link-class-${lastAddedIndex}`
        );
        if (newLinkInput) {
